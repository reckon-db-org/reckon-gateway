...
<p align="center">
  <a href="https://codeberg.org/reckon-db-org/reckon-gateway/releases"><img src="https://img.shields.io/badge/release-v0.6.2-1e40af" alt="release"/></a>
...
```bash
podman pull ghcr.io/reckon-db-org/reckon-gateway:0.6.2
```
...
podman run -d --name reckon-gw \
  ...
  ghcr.io/reckon-db-org/reckon-gateway:0.6.2
```
...
|---|---|
| `reckon_gateway` | 0.6.2 |
| `reckon_gater` (deps) | ~> 2.2 |
| `reckon_db` (deps, opt) | ~> 3.0 |
| Erlang/OTP | 27+ |

Pin to the semver tag (`:0.6.2`) for reproducible deploys; `:latest` tracks `main`.
